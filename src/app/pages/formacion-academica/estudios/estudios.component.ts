...
export class EstudiosComponent implements OnInit {
  ...
  element_datos: Estudios[] = [];
  paises: Pais[] = []
  universidades: Universidad[] = [];
  subfactorArray: Subfactor[] = [];
  foto: File
  gradoObjeto: Estudios = new Estudios();
  // idusuarioForParam;
  myId = uuidv4();
  imgCodified: string;
  dataSource = null;
  ...
  ngOnInit(): void {
    ...
    this.personaservice.getsubfactor(4).subscribe(subfactores=>{
      this.subfactorArray = subfactores
  ...
  obtenerdatos(){
    this.gradoservice.getOneEstudios(this.iddocente).subscribe(estudioUser=>{
      this.element_datos = estudioUser;
      this.dataSource = new MatTableDataSource<any>(estudioUser);
  ...
  guardarLegajo() {
    ...
    if (this.form.valid) {
      ...
      console.log()
      this.gradoservice.createEstudios(this.gradoObjeto).subscribe(data => {

        console.log(data, "ESTE GRADO A SIDO INGRESADO");
        this.personaservice.guardarimagen(this.foto, nombrenuevo);
        Swal.fire({
          icon: 'success',
          title: 'ESTUDIOS GUARDADO CORRECTAMENTE.',
