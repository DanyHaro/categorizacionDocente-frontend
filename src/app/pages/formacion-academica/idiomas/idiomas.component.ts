...
export class IdiomasComponent implements OnInit {
  ...
  element_datos: Idiomas[] = [];
  paises: Pais[] = []
  universidades: Universidad[] = [];
  subfactorArray: Subfactor[] = [];
  foto: File
  gradoObjeto: Idiomas = new Idiomas();
  // idusuarioForParam;
  myId = uuidv4();
  imgCodified: string;
  dataSource = null;
...
iddocente;
  ...
  ngOnInit(): void {
    ...
    this.personaservice.getsubfactor(5).subscribe(subfactores=>{
      this.subfactorArray = subfactores
  ...
  obtenerdatos(){
    this.gradoservice.getOneIdiomas(this.iddocente).subscribe(gradoOfuser=>{
  ...
  guardarLegajo() {
    ...
    if (this.form.valid) {
      ...
      console.log()
      this.gradoservice.createIdiomas(this.gradoObjeto).subscribe(data => {
